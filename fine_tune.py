...
parser.add_argument("--nbTargets", type=int, default=6, help="Number of images to output")
parser.add_argument("--depthFactor", type=int, default=1, help="Factor for the capacity of the network")
parser.add_argument("--loss", type=str, default="render", choices=["l1", "specuRough", "render", "flatMean", "l2", "renderL2"], help="Which loss to use instead of the L1 loss")
...
def load_examples(input_dir, shuffleValue):
    ...
    with tf.name_scope("load_images"):
        ...
        # shape = ?, 6, 256, 256, 3
        # second dimension is the number of targets, 0-3 normals, 3-6 baseColor, 6-9 roughness, 9-12 metallic, 12-15 specular, 15-18 ao
        # roughness
...
def create_model(inputs, targets, reuse_bool = False):
    ...
    outputedMetallic = outputs[:,:,:,6]
    outputedSpecular = outputs[:,:,:,7]
    outputedAo = outputs[:,:,:,8]
    ...
    outputedMetallicExpanded = tf.expand_dims(outputedMetallic, axis = -1)
    ...
    outputedAoExpanded = tf.expand_dims(outputedAo, axis = -1)
    reconstructedOutputs =  tf.concat([normNormals, outputedDiffuse, outputedRoughnessExpanded, outputedRoughnessExpanded, outputedRoughnessExpanded, 
                                       outputedMetallicExpanded, outputedMetallicExpanded, outputedMetallicExpanded, outputedSpecularExpanded, outputedSpecularExpanded, outputedSpecularExpanded, 
                                       outputedAoExpanded, outputedAoExpanded, outputedAoExpanded], axis=-1)
...
def append_index(filesets, output_dir = a.output_dir, step=False):
    print("shape of filesets : " + str(np.shape(filesets)))
    index_path = os.path.join(output_dir, "index.html")
    if os.path.exists(index_path):
        index = open(index_path, "a")
    else:
        mapnames = ["normals", "baseColor", "roughness", "metallic", "specularLevel", "ambientOcclusion"]
...
def main():
    ...
    if a.mode == "train":
        evalExamples = load_examples(a.input_dir.rsplit('/',1)[0] + "/test", False)
            ...
            try:
                ...
                for step in range(max_steps):
                    ...
                    if should(a.summary_freq):
                        ...
                        print("recording summary" + str(global_step))
...
def tf_render_diffuse_Substance(diffuse, metallic, ao):
    return diffuse * (1.0 - metallic) * ao / math.pi
...
def tf_Render(svbrdf, wi, wo, includeDiffuse = True):
    ...
    baseColor = squeezeValues(deprocess(svbrdf[:,:,:,3:6]), 0.0,1.0)
    normals = svbrdf[:,:,:,0:3]
    metallic = squeezeValues(deprocess(svbrdf[:,:,:,9:12]), 0.0, 1.0)
    roughness = squeezeValues(deprocess(svbrdf[:,:,:,6:9]), 0.0, 1.0)
    roughness = tf.maximum(roughness, 0.001)
    specular = squeezeValues(deprocess(svbrdf[:,:,:,12:15]), 0.0, 1.0)
    ao = squeezeValues(deprocess(svbrdf[:,:,:,15:18]), 0.0, 1.0)
    ...
    f0 = 0.16 * (1.0 - metallic) + baseColor * metallic

    # print("shape of svbrdf", svbrdf.get_shape())
    # print("shape of metallic", metallic.get_shape())
    # print("shape of ao", ao.get_shape())
    # print("shape of diffuse", baseColor.get_shape())

    diffuse_rendered = tf_render_diffuse_Substance(baseColor, metallic, ao)
    ...
    F_rendered = tf_render_F_GGX_Substance(f0, tf.maximum(0.0, VdotH))
    ...
    if includeDiffuse:
        result = result + diffuse_rendered * specular
    ...
    return [result, D_rendered, G_rendered, F_rendered, diffuse_rendered, baseColor]
